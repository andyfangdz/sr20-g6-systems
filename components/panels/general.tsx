...
export function Lighting() {
  ...
  return (
    <>
      <p className="lead">Exterior lighting is all LED: each wingtip carries a position light, a strobe, a white aft position light and a leading-edge landing/recognition light, so there is no tail light. A landing light sits in the lower cowl and ice inspection lights shine on the wing leading edges. Inside, convenience lighting (dome, baggage, footwell, entry-step) runs straight from BAT 1 through the CONV bus.</p>
      ...
      <Ctl>
        ...
        <Readouts items={[["Nav + aft position", ext(L.nav, x.nav)], ["Strobes", ext(L.strobe, x.strobe)], ["Landing + wingtip", ext(L.land, x.land)], ["Ice inspection", ext(L.ice, x.ice)]]} />
      </Ctl>
      ...
      <H3>Notes</H3>
      <Notes items={["There is no tail light: the rearward white position light is built into each wingtip trailing edge and comes on with NAV.", "LAND lights the lower-cowl landing light and both wingtip landing/recognition lights together.", "Ice inspection lights are for checking the leading edges at night. Many pilots use them only for quick checks because they cost night vision."]} />
      <H3>Power</H3>
